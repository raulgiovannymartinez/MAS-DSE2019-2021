...
rdd_tweets_userid_text = rdd_tweets.map(lambda x: (x['user']['id_str'], x['text']))

# print 3 elements from the rdd
rdd_tweets_userid_text.take(3)
...
users_distinct = rdd_tweets_userid_text.map(lambda x: x[0]).distinct()
...
# convert partition dictionary to rdd
rdd_partition = sc.parallelize(list(partition.items()))

# find users that belong to group 7, not in any partition
rdd_userid_group7_text = rdd_tweets_userid_text.subtractByKey(rdd_partition).map(lambda x: (x[0], 7, x[1]))

# create tuple (userid, group, tweet text) by combining groups 0 to 7 - cache to memory to use later in part 3
rdd_userid_group0to7_text = rdd_tweets_userid_text.join(rdd_partition).map(lambda x: (x[0], x[1][1], x[1][0]))                                .union(rdd_userid_group7_text).cache()        

# find the pair RDD tuple list (group, tweet count)
rdd_group0to7_tweetcounts = rdd_userid_group0to7_text.map(lambda x: (x[1], 1)).reduceByKey(lambda x,y: x+y)

# collect rdd 
counts_per_partition = rdd_group0to7_tweetcounts.collect()
...
# find tuple (userid, group, token) by applying tokenize method
rdd_userid_group0to7_token = rdd_userid_group0to7_text.map(lambda x: ((x[0],x[1]), (x[2])))                                .flatMapValues(tok.tokenize).map(lambda x: (x[0][0], x[0][1], x[1])).cache()

# count number of distinct tokens
num_of_tokens = rdd_userid_group0to7_token.map(lambda x: x[2]).distinct().count()
...
# find (token, count) for tokens mentioned by at least 100 users, count contribution is user per token
rdd_token_count_more100users = rdd_userid_group0to7_token.map(lambda x: (x[2], x[0])).distinct()                                .map(lambda x: (x[0],1)).reduceByKey(lambda x,y: x+y)                                .filter(lambda x: x[1]>=100).cache()

# find number of frequent tokens 
num_freq_tokens = rdd_token_count_more100users.count()

# sort, collect, and show top 20 most frequent tokens
top20 = rdd_token_count_more100users.sortBy(lambda x: x[1], ascending = False).collect()[:20]
...
# get tuple (token, group, count) for all tokens and users
rdd_token_group_count = rdd_userid_group0to7_token.distinct().map(lambda x: ((x[2],x[1]),1))                            .reduceByKey(lambda x,y: x+y).map(lambda x: (x[0][0], (x[0][1],x[1])))

# filter tweets with at least 100 user mentions
rdd_token_group_count_more100users = rdd_token_group_count.join(rdd_token_count_more100users)

# calculate the relative popularity for each tweet and group
rdd_relative_popu = rdd_token_group_count_more100users                        .map(lambda x: (x[1][0][0], [(x[0], get_rel_popularity(x[1][0][1], x[1][1]))]))
    
# find top 10 most popular for each group
rdd_popular10 = rdd_relative_popu.reduceByKey(lambda x,y: x+y).map(lambda x: x[1])                .map(lambda z: sorted(z, key = lambda x: (x[1], x[0]), reverse = True)).map(lambda x: x[:10])
    
# collect data
popular_10_in_each_group = rdd_popular10.collect()
