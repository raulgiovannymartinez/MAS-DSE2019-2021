...
#1. Make RDD from the data in the file given by the file path present in hw2-files.txt. (hint: use the textFile() method)
for f in file_path:
    rdd = sc.textFile(f)
...
#     raise NotImplementedError()
    ...
    # return boolean if line is json format and contains created_at field
    try:
...
# filter invalid tweets and convert to json format
rdd_tweets = rdd.filter(safe_parse).map(json.loads)

# create tuple of id_str and text
rdd_userid_text = rdd_tweets.map(lambda x: (x['user']['id_str'], x['text']))

# print 3 elements from the rdd
rdd_userid_text.take(3)
...
# count distinct users in valid tweets
users_distinct = rdd_userid_text.map(lambda x: x[0]).distinct()
users_count = users_distinct.count()
...
# get group number from the user partition pickle file, assign to group 7 if not present
user_text_partition = []
for user, text in rdd_userid_text.collect():
    
    try:
        group = partition[user]
    except:
        group = 7
        
    user_text_partition.append((user, text, group))

# convert each user to 1 for counting, switch group no. as key
user_partition_ones = sorted([(v2,1) for k,v1,v2 in user_text_partition])
    
# find the number of posts for each partition
counts_per_partition = [(k, len(list(v))) for k, v in itertools.groupby(user_partition_ones, lambda x: x[0])]

# put results into an rdd, sorted by key
rdd_counts_per_partition = sc.parallelize(counts_per_partition).sortByKey(ascending=True)
...
# tokenize all tweets text and form (user, token) list
token_user_group = []
for user, text, group in user_text_partition:
    for token in tok.tokenize(text):
        token_user_group.append((token, user, group))

# count number of distinct tokens
unique_tokens = np.unique([i for i,j,k in token_user_group])
num_of_tokens = len(unique_tokens)
...
# find (token, No users) 
token_user = [(i,j) for i,j,k in token_user_group]
counts_user_per_token = [(k, len(np.unique([i[1] for i in v]))) for k, v in 
                         itertools.groupby(sorted(token_user), lambda x: x[0])]

# filter out tokens mentioned by less than 100 users
counts_user_per_token_more100users = [i for i in counts_user_per_token if i[1] >= 100]

# find number of frequent tokens that pass the 100 users filter
num_freq_tokens = len(counts_user_per_token_more100users)

# sort and show top 20 most frequent tokens
top20 = sorted(counts_user_per_token_more100users, key = lambda x: x[1], reverse = True)[:20]
...
# define dictionary to save popular tokens
popular_10_in_each_group = {}

# loop over every group
for group in range(8):
    
    # get the (token, user) for all and for each group
    token_user_allgroups = [(i,j) for i,j,k in token_user_group]
    token_user_bygroup = [(i,j) for i,j,k in token_user_group if k == group]
    
    # get (group, user no.) for all and for each group
    total_mentions = [(k, len(np.unique([i[1] for i in v]))) for k, v in 
                      itertools.groupby(sorted(token_user_allgroups), lambda x: x[0])]
    partition_mentions = dict([(k, len(np.unique([i[1] for i in v]))) for k, v in 
                               itertools.groupby(sorted(token_user_bygroup), lambda x: x[0])])
    
    # filter out the ones with less than 100 user counts, for all user counts groups
    total_mentions_more100users = [i for i in total_mentions if i[1] >= 100]
    
    # calculate the relative popularity log ratio
    popular = []
    for i,j in total_mentions_more100users:
        try: # continue to next token when is not present in current group
            relative_popu = get_rel_popularity(j, partition_mentions[i])
            popular.append((i, relative_popu))
        except:
            continue
            
    # sort using (-p, t) as the key and keep top 10
    popular = [(k,-v) for k,v in popular]
    popular_10 = sorted(popular, key = lambda x: (x[1], x[0]), reverse = True)[:10]
    
    # create dictionary with each group as key
    popular_10_in_each_group[group] = popular_10
